...
import vn.com.sonca.ColorLyric.MagicTextView.Shadow;
...
public class SentenceTextView extends TextView {
	...
	public void setSententce(SentenceSong sententce) {
		this.sententce = sententce;
	}
	...
	public SentenceTextView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs);
	}

	public SentenceTextView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void setWords(ArrayList<WordSong> words, int lenght) {
		this.words = words;

		float sizeWidth = lenght;
		float firstWord = 0;

		maxLength = lenght;

		unit = (sizeWidth / this.getText().length());
		for (int i = 0; i < this.words.size(); i++) {
			this.words.get(i).setSizeStart(firstWord);
			float length = this.words.get(i).getWord().length() * unit;
			this.words.get(i).setSizeLength(length);
			firstWord = firstWord + length + unit;
		}
		currentWord = 0;
		SentenceTextView.this.getLayoutParams().width = 0;
	}
	...
	public void init(AttributeSet attrs){
		...
		if(attrs != null){
			...
			if(a.hasValue(R.styleable.MagicTextView_background2)){
				Drawable background = a.getDrawable(R.styleable.MagicTextView_background2);
				if(background != null){
					this.setBackgroundDrawable(background);
				}else{
					this.setBackgroundColor(a.getColor(R.styleable.MagicTextView_background2, 0xff000000));
				}
			}
