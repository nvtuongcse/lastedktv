...
public class ButtonScoreOnOff extends View {
	
	private Paint paintMain = new Paint(Paint.ANTI_ALIAS_FLAG);
	private TextPaint textPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
	
	public ButtonScoreOnOff(Context context) {
		super(context);
		initView(context);
	}

	public ButtonScoreOnOff(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
		initView(context);
	}

	public ButtonScoreOnOff(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initView(context);
	}
	
	private OnScoreOnOffCommandListener listener;
	public interface OnScoreOnOffCommandListener{
	...
	public void setOnScoreOffCommandListener(OnScoreOnOffCommandListener listener){
	...
	private void initView(Context context) {
		...
		title = getResources().getString(R.string.command_4h);
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		titleY = (float) (0.25*h - 0.1*titleS);
	...
	protected void onDraw(Canvas canvas) {
		...
		if(MyApplication.intColorScreen == MyApplication.SCREEN_BLUE || MyApplication.intColorScreen == MyApplication.SCREEN_KTVUI){
			...
			switch (stateScore) {
			case 0:
				textState = getResources().getString(R.string.main_left_5a);
				break;
			case 1:
				textState = getResources().getString(R.string.main_left_5b);
				break;
			case 2:
				textState = getResources().getString(R.string.main_left_5c);
				break;
			default:
				break;
			}
			textPaint.setStyle(Style.FILL);
			textPaint.setTextSize(staS);
			textPaint.setColor(color_01);
			canvas.drawText(textState.toLowerCase(), staX, staY, textPaint);
			switch (statePercent) {
			case 0:
				textState = getResources().getString(R.string.command_6x) + " " + getResources().getString(R.string.command_6b);
				break;
			case 1:
				textState = getResources().getString(R.string.command_6x) + " " + getResources().getString(R.string.command_6c);
				break;
			case 2:
				textState = getResources().getString(R.string.command_6x) + " " + getResources().getString(R.string.command_6a);
				break;
			default:
				break;
			}
				...
				case MotionEvent.ACTION_UP:{
					...
					if(rectBut1 != null){
						if(x >= rectBut1.left && x <= rectBut1.right &&
							...
							if(listener != null){
								if(flagBlock){
									listener.OnShowUpdateFirmware();	
								} else {
									listener.OnShowPercent();	
								}	
							}
							invalidate();
							break;
