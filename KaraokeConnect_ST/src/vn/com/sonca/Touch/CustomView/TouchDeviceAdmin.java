...
public class TouchDeviceAdmin extends View {
	...
	public static int LEFT_SLIDER = 3;
	public static int ADMIN_CONTROL = 4;
	public static int HIW_SETTING = 5;
	public static int GIAODIEN = 6;
	private int Layout = 0;

	public TouchDeviceAdmin(Context context) {
		super(context);
		initView(context);
	}

	public TouchDeviceAdmin(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
		initView(context);
	}

	public TouchDeviceAdmin(Context context, AttributeSet attrs, int defStyle) {
	...
	private void initView(Context context) {
		...
		textLabel1 = getResources().getString(R.string.adminfrag_0a);
		textLabel2 = getResources().getString(R.string.adminfrag_0b);
		textLabel3 = getResources().getString(R.string.adminfrag_0c);
		nhapmatkhau = getResources().getString(R.string.adminfrag_1);
		textLeft = getResources().getString(R.string.adminfrag_2);
		textRight = getResources().getString(R.string.adminfrag_3);
		
	}

	private OnDeviceAdminListener listener;

	public interface OnDeviceAdminListener {
		public void OnSendPass(String pass);
		public void OnBackLayout(int flag);
	}

	public void setOnDeviceAdminListener(OnDeviceAdminListener listener) {
		this.listener = listener;
	}

	public void setDataDevice(String ip, String name, int layout) {
		ipDevice = ip;
		nameDevice = name;
		Layout = layout;
	}
			...
			if (Layout == USER) {
				if(true){
					if (MyApplication.bOffUserList) {
						textLabel = getResources().getString(R.string.onoff_user_1);
					} else {
						textLabel = getResources().getString(R.string.onoff_user_2);
					}	
				} else {
					if (MyApplication.bOffUserList) {
						textLabel = getResources().getString(R.string.adminfrag_4a);
					} else {
						textLabel = getResources().getString(R.string.adminfrag_4b);
					}
				}
				
			} else if (Layout == HELLO) {
				textLabel = textLabel1;
			} else if (Layout == GIAODIEN) {
				textLabel = textLabel3;
			} else if (Layout == LEFT_SLIDER) {
				textLabel = getResources().getString(R.string.adminfrag_8);
			} else if (Layout == ADMIN_CONTROL) {
				textLabel = getResources().getString(R.string.adminfrag_9);
			} else if (Layout == HIW_SETTING) {
				textLabel = getResources().getString(R.string.adminfrag_10);
			}
			textPaint.setTextSize(labelS);
			float siz = textPaint.measureText(textLabel);
			labelX = widthLayout / 2 - siz / 2;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP: {
			...
			if(isMove == true){
				isMove = false;
				invalidate();
				if(System.currentTimeMillis() - longtimersync >= 2000){
					if (listener != null && password.length() >= 4) {
