...
public class TouchMyGroupSong extends View implements OnLongClickListener {
	...
	public TouchMyGroupSong(Context context) {
		super(context);
		initView(context);
	}

	public TouchMyGroupSong(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
		initView(context);
	}

	public TouchMyGroupSong(Context context, AttributeSet attrs, int defStyle) {
	...
	public interface OnMyGroupSongListener {
		public void OnSingerLink(boolean isMIDI , String nameSinger, int[] idSinger);
	...
	public void setOnMyGroupSongListener(OnMyGroupSongListener listener){
		...
		if (MyApplication.intColorScreen == MyApplication.SCREEN_BLUE || MyApplication.intColorScreen == MyApplication.SCREEN_KTVUI) {
			...
			if (MyApplication.intWifiRemote == MyApplication.SONCA) {
				...
				if(MyApplication.intSvrModel == MyApplication.SONCA_SMARTK && song.isYoutubeSong()){
					...
					float vuong = (float) (0.17*heightLayout);
					rectFavourite = new Rect(
							(int)(offsetX - vuong), 
							(int)(offsetY - vuong), 
							(int)(offsetX + vuong), 
	...
	public boolean onLongClick(View view) {		
		...
		if(listener != null && !idSong.equals("") && isEnableLongClick == true){
			isLongClick = false;
			listener.OnShowLyric(position, idSong5);
		}
		return false;
	}
    ...
    public boolean onTouchEvent(MotionEvent event) {
    	...
    	if(event.getAction() == MotionEvent.ACTION_UP && isLongClick){
    		...
    		if (offsetX > FirstSong && offsetX < SingerLink) {
    			if (listener != null) {
    				int ScreenX = (int) offsetTouchX;
    				int ScreenY = (int) offsetTouchY;
    		...
    		if (offsetX >= SingerLink && offsetX <= widthLayout &&
    			offsetY >= 0 && offsetY <= heightLayout/2) {
    			if (listener != null) {
    				if(!textID.equals("000000 | -")){
						if(ismedia == MEDIA_TYPE.MIDI){
...
*/
	...
	private String replaceAll(String stringIn){
