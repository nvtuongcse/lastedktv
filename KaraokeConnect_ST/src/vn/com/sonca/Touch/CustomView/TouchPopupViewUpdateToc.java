...
public class TouchPopupViewUpdateToc extends View {
	...
	public static final int LAYOUT_PARSESUCCESS = 8;
	public static final int LAYOUT_UPDATE_VIDEO = 9;
	...
	public TouchPopupViewUpdateToc(Context context) {
		super(context);
		initView(context);
	}

	public TouchPopupViewUpdateToc(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
		initView(context);
	}

	public TouchPopupViewUpdateToc(Context context, AttributeSet attrs, int defStyle) {
	...
	private OnPopupUpdateTocListener listener;

	public interface OnPopupUpdateTocListener {
		public void OnUpdateTocYes();

		public void OnUpdateTocNo();
	}

	public void setOnPopupUpdateTocListener(OnPopupUpdateTocListener listener) {
	...
	protected void onDraw(Canvas canvas) {
		...
		// BACKVIEW
		/*
		 * popupLayout = LAYOUT_PARSESUCCESS; MyApplication.intColorScreen =
		...
		if (MyApplication.intColorScreen == MyApplication.SCREEN_BLUE || MyApplication.intColorScreen == MyApplication.SCREEN_KTVUI) {
			if (popupLayout == LAYOUT_UPDATE) {
				...
				text = titleUpdate;
				if (text.equals("")) {
					text = getResources().getString(R.string.popup_uptoc_0);
				}
				float textWidth = mainPaint.measureText(text);
				canvas.drawText(text, widthLayout / 2 - textWidth / 2, KT1Y,
						mainPaint);

				text = getResources().getString(R.string.popup_uptoc_1);
				...
				canvas.drawText(text, (KD2R + KD2L) / 2 - textWidth / 2, KT4Y,
						mainPaint);
			} else if (popupLayout == LAYOUT_UPDATE_VIDEO) {
				KT1Y = 44 * heightLayout / 200;
				KT2Y = 75 * heightLayout / 200;
				KT3Y = 105 * heightLayout / 200;
				...
				text = getResources().getString(R.string.popup_upall_3);
				textWidth = mainPaint.measureText(text);
				canvas.drawText(text, widthLayout / 2 - textWidth / 2, KT2Y,
						mainPaint);

				text = getResources().getString(R.string.popup_upall_4);
				textWidth = mainPaint.measureText(text);
				canvas.drawText(text, widthLayout / 2 - textWidth / 2, KT3Y,
						mainPaint);
				// ----------YES BORDER-----------//
				...
				drawable.draw(canvas);
				// ----------YES NO TEXT-----------//
				text = getResources().getString(R.string.popup_upall_c3);
				textWidth = mainPaint.measureText(text);
				canvas.drawText(text, (KD1R + KD1L) / 2 - textWidth / 2, KT4Y,
						mainPaint);
				...
				text = getResources().getString(R.string.popup_uppic2c);
				float textWidth = mainPaint.measureText(text);
				canvas.drawText(text, widthLayout / 2 - textWidth / 2, KT2Y,
						mainPaint);
				...
				text = getResources().getString(R.string.popup_uptoc_2a);
				float textWidth = mainPaint.measureText(text);
				canvas.drawText(text, widthLayout / 2 - textWidth / 2, KT2Y,
						mainPaint);

				KT3Y = 120 * heightLayout / 200;
				text = getResources().getString(R.string.popup_uptoc_2b);
				textWidth = mainPaint.measureText(text);
				canvas.drawText(text, widthLayout / 2 - textWidth / 2, KT3Y,
						mainPaint);
			} else if (popupLayout == LAYOUT_PARSESUCCESS) {
				// ----------KHUNG-----------//
				drawable = drawKhung;
				drawable.setBounds(0, 0, widthLayout, heightLayout);
				drawable.draw(canvas);
				...
				text = getResources().getString(R.string.popup_uptoc_3a);
				float textWidth = mainPaint.measureText(text);
				canvas.drawText(text, widthLayout / 2 - textWidth / 2, KT2Y,
						mainPaint);

				KT3Y = 120 * heightLayout / 200;
				text = getResources().getString(R.string.popup_uptoc_3b);
				textWidth = mainPaint.measureText(text);
				canvas.drawText(text, widthLayout / 2 - textWidth / 2, KT3Y,
						mainPaint);
			}
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (popupLayout == LAYOUT_UPDATE || popupLayout == LAYOUT_UPDATEDAUMAY || popupLayout == LAYOUT_UPDATE_VIDEO) {
			...
			case MotionEvent.ACTION_UP: {
				...
				if (x >= KD1L && x <= KD1R && y >= KD1T && y <= KD1B) {
					if (listener != null) {
						listener.OnUpdateTocNo();
					}
				}

				if (x >= KD2L && x <= KD2R && y >= KD2T && y <= KD2B) {
					if (listener != null) {
						listener.OnUpdateTocYes();
					}
				}
