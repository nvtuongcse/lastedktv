...
public class TopHiWView extends View {
	
	private Paint mainPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
	private Paint mainText = new Paint(Paint.ANTI_ALIAS_FLAG);
	
	public TopHiWView(Context context) {
		super(context);
		initView(context);
	}

	public TopHiWView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
		initView(context);
	}

	public TopHiWView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void initView(Context context) {
		titleButton = getResources().getString(R.string.hiw_title);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (MyApplication.intColorScreen == MyApplication.SCREEN_BLUE || MyApplication.intColorScreen == MyApplication.SCREEN_KTVUI) {
			...
			float xx = (float) (titleX - 0.5 * mainText
					.measureText(titleButton));
