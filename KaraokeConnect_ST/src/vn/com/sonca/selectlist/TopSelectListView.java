...
public class TopSelectListView extends View {
	
	private Paint mainPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
	private TextPaint textPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
	
	public TopSelectListView(Context context) {
		super(context);
		initView(context);
	}

	public TopSelectListView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
		initView(context);
	}

	public TopSelectListView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initView(context);
	}
	
	private OnTopSelectListViewListener listener;
	public interface OnTopSelectListViewListener {
		public void OnBackLayout();
	}
	
	public void setOnTopSelectListViewListener(OnTopSelectListViewListener listener) {
	...
	private void initView(Context context) {
		...
		label = getResources().getString(R.string.top_select_list);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if(MyApplication.intColorScreen == MyApplication.SCREEN_BLUE || MyApplication.intColorScreen == MyApplication.SCREEN_KTVUI){
			...
			labelX = (float) (0.5*widthLayout - 0.5*textPaint.measureText(label));
