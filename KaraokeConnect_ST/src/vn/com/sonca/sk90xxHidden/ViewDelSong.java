...
//	private Spannable spanNemberID = new SpannableString("888000 | 33888");
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		offsetX = (float) (0.015*w);
		offsetY = (float) (0.82*h);
		vuongH = (float) (0.1*h);
		vuongW = vuongH*25/30;
		rectBlock = new Rect(
				(int)(offsetX - vuongW), 
		...
		*/
		...
		if (MyApplication.intColorScreen == MyApplication.SCREEN_BLUE) {
			...
			if (MyApplication.intWifiRemote == MyApplication.SONCA) {
				...
				if(song.isHidden()){
					drawable = getResources().getDrawable(R.drawable.khung_hid_xam);
...
//					canvas.drawLine(0, getHeight() / 2, getWidth(), getHeight() / 2, paintSimple);
					...
					if (MyApplication.bOffFirst == true) {
						if (isTouch) {
							...
							if(song.isHidden()){
								FR = (float) (1.0 * CircleR);
								drawable = getResources().getDrawable(R.drawable.touch_icon_block);
								if (drawable != null) {
									drawable.setBounds((int) (CircleX - FR), (int) (CircleX - FR), (int) (CircleX + FR),
											(int) (CircleX + FR));
									drawable.draw(canvas);
								}
							} else {
					...
					if (spanNemberID != null) {
						if (!spanNemberID.toString().equals("000000 | -")) {
							...
							if (!isUser) {
								...
								if(song.isHidden()){
									mainText.setARGB(255, 255, 224, 120);
								}
							}
							builderNumber.clear();
