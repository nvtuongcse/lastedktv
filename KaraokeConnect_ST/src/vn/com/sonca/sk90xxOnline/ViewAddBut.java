...
public class ViewAddBut extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		if(MyApplication.flagAutoDownloading || flagEmpty){
			drawXam.setBounds(rectImage);
			drawXam.draw(canvas);
		} else {
			if(title.equals(getResources().getString(R.string.dialog_90xx_online_2)) && MyApplication.intDownloadRemain == 0){ // huy an
				drawXam.setBounds(rectImage);
				drawXam.draw(canvas);
			} else {
				if(isClick == false){
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if(listener != null && flagEmpty == false && MyApplication.flagAutoDownloading == false){
