...
public class ViewSearchAdd extends View {
	...
	private void initView(Context context) {
		...
		textHint = getResources().getString(R.string.dialog_90xx_online_8);
...
//		rectDelete.set((int)(0.485*w - 2*ww), tamY-hh, (int)(0.485*w), tamY+hh);
	...
	protected void onDraw(Canvas canvas) {
		...
		textPaint.setStyle(Style.FILL);
		String str = getResources().getString(R.string.dialog_90xx_online_6);
		...
		textPaint.setTextSize(KS2);
		textPaint.setStyle(Style.FILL);
		str = getResources().getString(R.string.dialog_90xx_online_7);
    ...
    public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP: {
			float x = event.getX();
			float y = event.getY();
			if(x >= KR3L && x <= KR3R && y >= KR3T && y <= KR3B && !MyApplication.flagAutoDownloading){
				stateFilter = ONLINE_SEL;
				invalidate();
				if(listener != null){
	...
	public static final int ONLINE_SEL = 1;
