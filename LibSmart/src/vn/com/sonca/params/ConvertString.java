...
public class ConvertString {
	...
	public byte[] ConvertUnicodeToTCVN(String unicodeString) {
		Integer idx = 0;
		List<Integer> UniCharList = new ArrayList<Integer>();

		UniCharList.addAll(Arrays.asList(UNICODE_LUT));
		List<Byte> list = new ArrayList<Byte>();

		byte[] array = new byte[1];
		try {
			for (idx = 0; idx < unicodeString.length(); idx++) {
				int intIDX = unicodeString.charAt(idx);
				if(UniCharList.contains(intIDX)){
					list.add(ByteUtils.intToByte(TCVN_LUT[UniCharList.indexOf(intIDX)]));
				} else {
					list.add(ByteUtils.intToByte(intIDX));
				}
			}
